...
function encodeToRot13(str) {
	...
	for (var i = 0; i < str.length; i++) {
		if (input.indexOf(str[i]) == -1) {
			...
			result += output.charAt(input.indexOf(str[i]));
		}
	}
	return result;
}
...
function getCardId(value) {
    let templ = 'A234567891JQK';
	let result = templ.indexOf(value[0]);
	if (value.includes('♦')) {result += 13}
	if (value.includes('♥')) {result += 26}
	if (value.includes('♠')) {result += 39}
	return result;
}
